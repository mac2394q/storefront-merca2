...
//$pedidosActuales = $objPedidosController->listadoPedidosTerceroMes($arrayRequest = ["idcliente" => $_GET['id']]);
                                                    ...
                                                    <table>
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th>historial Mensual de Facturas</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                        ";
                                        foreach($objTercerosRed  as $key => $value ){ 
                                            echo "
                                                <tr>
                                                    ...
                                                    $pedidosClientesActuales =$objPedidosController->listadoPedidosTerceroMes($arrayRequest = ["idcliente" => $objTercerosRed[$key]->getRowid()]);
                                                    if($pedidosClientesActuales != false){
                                                        $countFacturas = 0;
                                                        $sumDinero = 0;
                                                        $sumPuntos=0;
                                                        echo "<ul>";

                                                        $puntosAcumuladoFactura = 0;
                                                        $dineroAcumulado = 0;
                                                        foreach($pedidosClientesActuales  as $key2 => $value ){ 
                                                            
                                                            $objPedidos_ = $objPedidosController->obtenerPedidoId($arrayRequest = ["idpedido" => $pedidosClientesActuales[$key2]->getRowid()]);

                                                            foreach ($objPedidos_->getArraySubCategorias() as $key3 => $value) {
                                                                
                                                                $precioVenta =$objPedidos_->getArraySubCategorias()[$key3]->getTotal_ttc();
                                                                $precioCompra =intval($objPedidos_->getArraySubCategorias()[$key3]->getTotal_ttc())/2;
                                                                $ganancia = intval($precioVenta ) - intval($precioCompra);
                                                                $porcentaje = (intval($ganacia)*100) /intval($precioVenta ) ;
                                                                $puntosConsumo =((intval($precioVenta ) /12)*intval($porcentaje ))/1000;
                                                                $puntosAcumuladoFactura+=$puntosConsumo;
                                                                $dineroAcumulado +=intval($puntosConsumo)*1000;

                                                            }
                                                            echo "<li>Factura ".$pedidosClientesActuales[$key2]->getRef()." - Acomulado puntos ".$puntosAcumuladoFactura." - Acumulado Dinero ".$dineroAcumulado."</li>";
                                                            $countFacturas++;
                                                        }
                                                        echo "<li>Numero de Facturas : ".$countFacturas."</li>";
                                                        echo "<li>Total acumulado de Puntos: Dinero Recaudado: </li>";
                                                        echo "</ul>";

                                                    }else{ echo "No hay facturas para este mes";}
